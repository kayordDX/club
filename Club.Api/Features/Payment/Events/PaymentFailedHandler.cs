...
public class PaymentFailedHandler(AppDbContext dbContext, ILogger<PaymentFailedHandler> logger) : IEventHandler<PaymentFailedEvent>
{
    ...
    {
        var payment = await _dbContext.Payment.FirstOrDefaultAsync(p => p.Id == eventModel.PaymentId, ct);

        if (payment is null)
        {
            _logger.LogWarning("PaymentFailedEvent: Payment {PaymentId} not found.", eventModel.PaymentId);
            return;
        }
