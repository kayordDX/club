...
{
    public static async Task<PaginatedList<T>> GetPagedAsync<T>(this IOrderedQueryable<T> query, QueryModel queryModel, CancellationToken ct)
        where T : class
    {
    ...
    private static async Task<(IQueryable<T> itemsQuery, int pageNumber, int pageSize, int count)> GetPagedResultAsync<T>(
        IQueryable<T> query,
    ...
    public static IQueryable<T> ApplyQueryResult<T>(IQueryable<T> query, QueryModel queryModel)
        where T : class
    {
        var queryKitConfig = new CustomQueryKitConfiguration();
        var queryKitData = new QueryKitData()
    ...
    public static IQueryable<T> ApplyQuery<T>(this IQueryable<T> source, QueryModel queryModel)
        where T : class
    {
